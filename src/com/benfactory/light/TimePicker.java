...
public class TimePicker extends FrameLayout {
	...
	// picker type
	private TimePickerType mPickerType;

	public enum TimePickerType {
		HMS,        // To display hours, minutes and seconds
		MS,         // To display only minutes and seconds
		MS15,       // To display only minutes and only every 15 seconds
		MS15MAX10   // To display only minutes and only every 15 seconds, up to 10 minutes
	};
	...
	public TimePicker(Context context, AttributeSet attrs, int defStyle) {
		...
		mSecondPicker.setOnValueChangedListener(new NumberPicker.OnValueChangeListener() {
			...
			@Override
			public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
				mCurrentSeconds = (mSecondPicker.getDisplayedValues()!=null?Integer.parseInt(mSecondPicker.getDisplayedValues()[newVal]):newVal);
				onTimeChanged();
	...
	public TimePickerType getmPickerType() {
		return mPickerType;
	}

	public void setPickerType(TimePickerType pickerType) {
		this.mPickerType = pickerType;
		updateTimerPickerDisplayAccordingToType();
	}

	private void updateTimerPickerDisplayAccordingToType() {
		// Update seconds display
		if(TimePickerType.MS15.equals(getmPickerType())||TimePickerType.MS15MAX10.equals(getmPickerType())){
			mSecondPicker.setMaxValue(3);
			mSecondPicker.setDisplayedValues(new String[]{"0", "15", "30", "45"});
		} else {
			mSecondPicker.setMaxValue(59);
		}
		...
		if(TimePickerType.MS.equals(getmPickerType())
			||TimePickerType.MS15.equals(getmPickerType())
			||TimePickerType.MS15MAX10.equals(getmPickerType())){
	...
	private void updateSecondsDisplay() {
		if(mSecondPicker.getDisplayedValues()==null){
			...
			mSecondPicker.setValue(Arrays.asList(mSecondPicker.getDisplayedValues()).indexOf(String.valueOf(mCurrentSeconds)));
		}
		mOnTimeChangedListener.onTimeChanged(this, getCurrentHour(), getCurrentMinute(), getCurrentSeconds());
	}
}
